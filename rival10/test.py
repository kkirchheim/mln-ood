...
from shared import train_mln_pseudo
...
def get_domain(atts) -> list:
    domain = []
    for att in atts:
        if att == "oe":
            domain.append([0, 1])
        if att == "label":
            domain.append(range(10))
        else:
            domain.append([0, 1])
    return domain
...
def fit_all_detectors(cfg, seed) -> List[Detector]:
    ...
    path = join(cfg.paths.predictions, f"data-{cfg.mln.train_on}-{seed:05d}.pt")
    ...
    if "ablation_n_rules" in cfg and cfg.ablation_n_rules:
        ...
        log.info(f"Limiting number of rules to {len(constraints)}")
    ...
    if cfg.use_oe:
        ...
        train_mln_pseudo(cfg, oe_mln, data_oe)
        torch.save(oe_mln.w.data, join(get_output_dir(), f"oe-mln-{seed}.pt"))

    data_normal = get_predictions(data, cfg.attributes[:-1])
    mln = MLN(constraints=constraints, domain=get_domain(cfg.attributes[:-1]))
    torch.nn.init.constant_(mln.w.data, cfg.mln.init)
...
def initialize_detectors(cfg, mln, seed, oe_mln=None) -> List[Detector]:
    ...
    vim = ViMDetector(
        w, b, attribute=cfg.target_att, target_att_index=cfg.target_att_index
    )
    dice = DICEDetector(
        w, b, attribute=cfg.target_att, target_att_index=cfg.target_att_index
    )
    maha = MahalanobisDetector(
        attribute=cfg.target_att, target_att_index=cfg.target_att_index
    )
    ...
    react = ReActDetector(head=head, attribute=cfg.target_att)
    she = SHEDetector(
        head=head, attribute=cfg.target_att, target_att_index=cfg.target_att_index
    )
    ...
    detectors: List[Detector] = [
        ...
        LogicDetector(constraints=mln.constraints, attributes=cfg.attributes),
        MLNDetector(mln, name="MLN", attributes=cfg.attributes),
        ...
        MLNMSPDetector(mln, mln_attributes=cfg.attributes, attribute=cfg.target_att),
        MLNReActDetector(
            mln,
            react,
            mln_attributes=cfg.attributes,
...
def evaluate_all(cfg, seed, detectors: List[Detector]) -> pd.DataFrame:
    ...
    for dataset_name in cfg.datasets:
        path = join(cfg.paths.predictions, f"data-{dataset_name}-{seed:05d}.pt")
        log.info(f"Evaluating on dataset '{dataset_name}' from '{path}'")
        ood_data = torch.load(path, weights_only=False)
        ood_data = {key.lower(): value for key, value in ood_data.items()}
