...
def fit_all_detectors(cfg, seed) -> List[Detector]:
    ...
    mln = MLN(constraints=constraints, domain=[range(2) for _ in cfg.attributes[:-1]])
...
def initialize_detectors(cfg, mln, seed, oe_mln=None) -> List[Detector]:
    ...
    she = SHEDetector(head=head, attribute=cfg.target_att)
    ...
    fitting_data = {key.lower(): value for key, value in fitting_data.items()}
...
def evaluate_all(cfg, seed, detectors: List[Detector]) -> pd.DataFrame:
    ...
    id_data = {key.lower(): value for key, value in id_data.items()}
