...
from compiler import ConstraintCompiler, Implies, And, Or, Xor, Not, BinaryVariable
...
def generate_all_constraints(attributes, depth=2):
    ...
    constraints = generate_all_expressions(
        depth, propositions=attribute_index_map.keys()
    )
...
def load_data(cfg, seed):
    ...
    data_train = {k.lower(): v.half() for k, v in data_train.items() if "-logits" in k}

    data_in = torch.load(
        join(cfg.paths.predictions, f"data-{cfg.validate_on}-{seed:05d}.pt"),
        weights_only=False,
    )
    data_in = {k.lower(): v.half() for k, v in data_in.items() if "-logits" in k}

    data_ood = torch.load(
        join(cfg.paths.predictions, f"data-{cfg.dataset}-{seed:05d}.pt"),
        weights_only=False,
    )
    data_ood = {k.lower(): v.half() for k, v in data_ood.items() if "-logits" in k}
